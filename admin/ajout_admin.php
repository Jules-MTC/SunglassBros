<script>
    function chargement() {
        window.open('http://localhost/SunglassBros/admin/conf_ajout_admin.php');
        setTimeout(function() {
            window.close();
    ...
    if (isset($_POST['g-recaptcha-response']) && !empty($_POST['g-recaptcha-response'])) {
        ...
        if ($result->success == true) {
            ...
            if ($result && mysqli_num_rows($result) > 0) {
                var_dump($email);
                $sql = "UPDATE usertb SET droits = 'admin' WHERE email = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("s", $email);
                /*  $stmt->execute();
                $result = $stmt->get_result(); */
                ...
                if ($stmt->execute()) {
                    ...
                    $error_msg = "Impossible de modifer les droits";
                }
            } else {
                $conn->close();
...
<center>
    ...
    <body>
        <div class="login-box">
            <h1>Ajout admin :</h1>
            <form name="ajout_admin" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                <div class="user-box">
                    <input type="email" name="email" required>
                    <label>Email du futur admin :</label>
                ...
                <a id="send" href="javascript:document.forms['ajout_admin'].submit()">Ajouter</a>
            </form>
        </div>
    </body>
</center>
